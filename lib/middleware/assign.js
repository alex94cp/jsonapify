		...
		async.waterfall([
			...
			function(parent, next) {
				...
				resview.findOne(transaction, filter, function(err, object) {
					...
					next(null, resview, object);
				});
			},
			function(resview, object, next) {
				resview.deserialize(transaction, req.body.data, object, function(err, data) {
					err ? next(err) : next(null, resview, data);
				});
			},
		], function(err, resview, object) {
			if (err) return next(err);
