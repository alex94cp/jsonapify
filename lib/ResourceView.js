...
function ResourceView(resource, fields, opts) {
	var self = this;
...
ResourceView.prototype.findOne = function(transaction, filter, callback) {
...
ResourceView.prototype.findMany = function(transaction, filter, callback) {
...
ResourceView.prototype.serialize = function(transaction, object, callback) {
	var resdata = {};
...
ResourceView.prototype.deserialize = function(transaction, resdata, object, callback) {
	async.each(this._writableFields, function(field, next) {
		var value = _.get(resdata, field.name);
		field.deserialize(transaction, value, object, next);
	}, function(err) {
