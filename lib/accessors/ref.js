...
Ref.prototype.serialize = function(object, response, cb) {
	...
	model.findOne(filter, function(err, related) {
		...
		async.parallel({
			id: async.apply(getField, related, refResource.id),
			type: async.apply(getField, related, refResource.type),
			data: refResource.serialize.bind(refResource, related, response),
